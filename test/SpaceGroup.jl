@testset "> Potential equivalents regression" begin do
...
@testset "> Point group operations" begin do
    parameters = [
    ...
    @testset "Cell: $cell" for (cell, numops) in parameters
        ...
        for op in ops
            ...
            @test op.offset --> roughly([0, 0, 0])
            transformation = inv(cell) * op.scalefwd * cell
            @test transformation --> all_integers
            @test abs(det(transformation)) --> roughly(1)
